...
export const allCards = createSelector(selectCardState, selectAllCards);
...
export const selectCardsLoading = createSelector(
  ...
  (loading, labelLoading, commentLoading) =>
    loading && labelLoading && commentLoading
);

export const selectCardsByColumnId = (columnId: string) =>
  createSelector(
    allCards,
    (cards) => cards && cards.filter((c) => c.columnId === columnId)
  );

export const clearFilterVisible = createSelector(cardFilters, (filters) => {
  if (!filters) {
    return false;
  }
...
export const selectCardsByColumnIdWithFilters = (columnId: string) =>
  createSelector(
      ...
      return cards
        .filter((card) =>
          assigneesFilter.length > 0
            ? assigneesFilter.includes(card.assigneeId)
            : card
        )
        .filter((card) =>
          labelsFilter.length > 0
            ? card.labels.some((l) => labelsFilter.includes(l))
            : card
        )
        .filter((card) =>
          typesFilter.length > 0 ? typesFilter.includes(card.type) : card
        );
    }
  );

export const selectLatestOrdinalId = createSelector(allCards, (cards) => {
  const cloned = [...cards];
